...
| `apiServerHost`                          | Address the HTTP API binds to. Defaults to localhost because, without TLS, the API sends its token over plaintext HTTP. Set to `0.0.0.0` to accept requests from other hosts, ideally with `apiServerTlsCertPath`/`apiServerTlsKeyPath` set or behind a TLS reverse proxy. | string  | `127.0.0.1`   | No       |                                               |
...
When `apiServerTlsCertPath` and `apiServerTlsKeyPath` are set, the API is served over HTTPS instead:

```
https://<apiServerHost>:<apiServerPort>
```

The API binds to `127.0.0.1` by default, so it is reachable only from the Homebridge host. Without TLS the token is sent over plaintext HTTP, so set `apiServerHost` to `0.0.0.0` only when you need remote access. For remote access, serve HTTPS directly by setting `apiServerTlsCertPath` and `apiServerTlsKeyPath` to a PEM certificate and key, or put a TLS reverse proxy in front of it.
